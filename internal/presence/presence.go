...
func (u Update) Settings() map[string]any {
	out := map[string]any{"status": u.Status}
	for _, a := range u.Activities {
		if a.Type != TypeCustom {
			continue
		}
		cs := map[string]any{"text": a.State, "expires_at": nil}
		if a.Emoji != nil && a.Emoji.Name != "" {
			cs["emoji_name"] = a.Emoji.Name
		}
		out["custom_status"] = cs
		break
	}
	return out
}
