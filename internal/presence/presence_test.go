...
func TestSettings(t *testing.T) {
	t.Parallel()
	u, err := Build(Options{Status: "dnd"})
	if err != nil || u.Settings()["status"] != "dnd" {
		t.Fatalf("%+v %v", u, err)
	}
	u, err = Build(Options{Status: "online", ActivityType: "custom", Name: "hi", Emoji: "🔥"})
	if err != nil {
		t.Fatal(err)
	}
	cs, _ := u.Settings()["custom_status"].(map[string]any)
	if cs["text"] != "hi" || cs["emoji_name"] != "🔥" {
		t.Fatalf("%+v", u.Settings())
	}
}
